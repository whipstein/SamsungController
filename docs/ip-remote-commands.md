...
This is the **archived command-testing guide** and protocol reference for 69 command families: the 52 families in the 2020 list plus 17 additional consumer-TV methods from the linked firmware notes. The testing UI described below has been removed; its URLs now open [Communication log](direct-ip-interface.md#communication-log). Normal settings live on [Menu](direct-ip-interface.md), without manual verification. Retained interrupted-operation records can be closed through the log's manual-recovery section. The numbered testing steps below describe an earlier development checkpoint, not the current interface.

**2-point white-balance correction:** the historical partial-channel examples below describe the low-level payload format, not a reliable TV update strategy. Observed R-Gain-only requests partly applied with an error, while B-Gain-only requests did not apply. Current Menu sends all six freshly read channels with only the edited target replaced, and stops if any original channel is unavailable. See [current 2-point behavior](direct-ip-interface.md#2-point-white-balance).
...
| Two-point white balance | `colorToneControl`, `WB2PointControl` | Tone choices; six gain/offset channels −50…50. Use a complete six-field request preserving freshly read peers; partial requests are unreliable. |
