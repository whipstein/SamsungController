...
With **Apply immediately**, repeated **+ / −** clicks enqueue each successive target, even while earlier clicks are awaiting the TV. For example, starting at 10, +, +, − requests 11, 12, then 11; targets are not collapsed into one write. The number displays the latest queued target without snapping back to an earlier reply. Each click retains the normal fresh-baseline checks, journal, independent readback, and calibration-row selector safeguards. Two-point writes still preserve all six channels. Wait for Apply continues to stage a single target without sending until Apply.

Numeric +/− buttons remain available during their adjustment queue; other edits, refreshes, remote keys, and connection changes cannot interleave. Fixed-size number fields, reserved status rows, stable current-value widths, and a reserved header-status line prevent button movement as requests begin/end. Availability layout is held steady during a queue; it is not permission to bypass prerequisite or context checks. Slider dragging and direct text edits wait until the running queue finishes.

**Stop**, a failed adjustment, or a changed reported TV context discards the remaining queued clicks. Earlier confirmed changes remain; no rollback or automatic retry is sent. An uncertain delivered write still needs review. Queues are memory-only, never resume after restart, and accept at most 256 outstanding clicks. Repeated clicks beyond min/max do not add redundant requests. Keep the physical signal unchanged, since not every external input condition is reported by the TV.
