...
2. Connect also preloads all 20-point percentages and Custom colors when their modes are already enabled, then opens Menu. These grids move their selector to read every row and restore it; no RGB values or modes change during loading. Inactive modes remain unchanged, with unavailable rows reported in connection-preload details. Missing fields remain **Not reported**, not defaults or values imported from a saved profile. Tab changes use the loaded values without restarting incomplete/unsupported reads. Stop cancels the preload; Refresh or reconnect explicitly to try again.
