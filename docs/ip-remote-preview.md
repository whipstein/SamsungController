...
This document preserves the **early staged diagnostic workflow**. The working branch now replaces the primary web interface with [direct Menu controls](direct-ip-interface.md), without manual verification gates. The old WebSocket/menu-key pages and macro editor are hidden. The testing screens described below have been removed; their URLs now open [Communication log](direct-ip-interface.md#communication-log), which retains manual recovery for interrupted operations. The owner approved the merge for v1.0.0; direct IP is now the default on `main`. Follow the current [README](../README.md) for normal installation and use; the numbered steps below are development history, not today's interface.
