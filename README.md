...
2. **Configuration:** the initial **Default** layout is ready automatically. Add and describe another named configuration whenever selections make rows appear, disappear, or move, such as `Game Mode off` and `Game Mode on`. Choose the active configuration before recording or navigating; it is also shown beside **Current menu** in the persistent header. Switching it resets the expected position to unknown.
...
4. **Record route:** record exceptional entry behavior, normally the `normal-video` anchor and the traversal from **Normal video** into **Settings**. Every accepted button is sent to the TV immediately and added to the draft. Once an entry traversal reaches a modeled submenu, SamsungController generates routes to its descendants from the ordered topology: it moves Down to the child row, presses Enter only for submenus, and omits rows disabled under the declared defaults. Record another explicit traversal only when a branch does not follow those rules. **Also record return to normal video** captures a target-specific exception with its forward route.
5. **Verify:** generated descendants are grouped by top-level branch. The page selects one longest representative route per branch and labels how many generated destinations it covers. Three accepted passes promote that entire group together, including its recorded seed traversal. Changing row order, control types, disabled defaults, or the seed keys automatically regenerates and invalidates only affected groups.
