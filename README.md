...
Choose **Stay on last adjusted item** to leave the final control visible for inspection, or **Exit to normal video** to run the verified return anchor afterward. Route verification proves that a row can be reached; it does not prove the displayed setting value because the Samsung remote channel does not report values. Visually confirm three different sliders to promote the common slider behavior for the saved TV address. Selection dropdowns are verified individually because each has its own ordered option list. Both results persist across restarts, and a verified control no longer asks for routine confirmation. Use **Reset sliders** or **Reset selections** if later results disagree. Saving different system-wide menu delays resets both kinds of coverage because those delays affect their key sequences. Switches continue to request visual confirmation. If the TV was adjusted outside SamsungController, first restore the declared defaults or update the definition before relying on relative changes.
