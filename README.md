...
Use **Enter current settings** when the TV already has settings you want SamsungController to treat as its starting point. Enter the values currently visible on the TV—or load a matching `.samsung-calibration.json` file into the current-settings draft—then name the state and select **Save entered values**. This workspace never calls the TV: its draft is separate from staged adjustments, and loading a state only replaces the prediction baseline. Named states—including indexed grid cells—stay in local application settings. **Save profile** separately stores desired target values from **Adjust TV**. **Reset to defaults** runs the chosen verified reset, keeps the TV at factory defaults, and replaces both the current prediction and saved target profile with the menu-definition defaults. **Reset & apply all** instead reapplies the saved calibration after resetting. Both are explicitly confirmed destructive operations; reset commands are never required by Display Verification.
