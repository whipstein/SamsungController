...
3. If the browser does not open, visit `http://127.0.0.1:5050`. Reopening the app reuses a responsive SamsungController tab instead of opening another. A running session keeps its page and unsent edits; a tab left after a server restart reloads at the same address. If no tab responds promptly, the app opens a new one. Browsers may require you to select the reused tab yourself.
4. **Closing the browser leaves the server running.** Use **Quit app** at the very top, beside **Stop**, on any page; stop any active TV operation first. Quit also asks SamsungController tabs to close (including when quitting the native Mac app). If your browser blocks automatic tab closure, a non-blocking **App stopped** page remains; reopen SamsungController to reuse that tab automatically. If your browser suspended it, select the old tab and click **Return to app**, or close it with **⌘W** on Mac / **Ctrl+W** on Windows/Linux. Tab reuse requires a page loaded from v1.1.2 or newer; refresh an older-version tab once after upgrading. No extra browser permission is needed. Return to app only checks server readiness; it does not start the server or send TV commands. Unrelated tabs stay open. No login/startup service is installed.
...
- **Mac updates and network permission:** quit the app, drag the new app to Applications and choose **Replace**, eject the DMG, then open the installed copy. Keep only one unpacked SamsungController app; old downloads can remain as DMGs/ZIPs. The app's bundle/signing identity stays the same across updates, so approval should carry forward. Multiple unpacked copies can confuse macOS's permission cache even while Local Network shows enabled. Re-pairing the TV does not fix an OS network-permission error. See Help → Troubleshooting.
...
6. Use **Stop** during an operation. Delivered commands are not undone. For an uncertain write, **Verify settings** appears beside Apply and Stop in the pinned top toolbar. Open it to inspect the affected values and saved originals, check the TV, and close the review explicitly. Closing sends no commands or retries; use **Refresh state** afterward before more adjustments. Interrupted recalls open their combined review in Recall settings.

The TV must be **on** for adjustments. Standby can still answer network queries with saved settings, then reject writes with “Method not found.” The app automatically checks power before each apply group, including when **Query first** is Off; this adds one read per group, not per RGB channel. If the TV reports Off, the attempted targets revert locally and a highlighted warning appears in the pinned toolbar. **Turn the TV on and try your adjustment again**—no Check power button, review flag, or reconnect is required. Unrelated pending edits are kept. Displays that do not report power can still use their supported settings. No automatic power-on, retries, or background power polling occur.

If the TV rejects an input or setting and readback confirms it did not change, its control returns to the confirmed value and the issue appears in the pinned toolbar. You can keep using the app. This is a local UI correction, not an automatic rollback command. Ambiguous delivered writes (such as a dropped connection or conflicting readback) still require review because the TV may have changed.
...
3. Click **Recall settings**, choose a **Saved state** from its dropdown, and review its values and omissions. Select the matching display, picture mode and signal first, then refresh. **Any input can be used**: input is not saved or required to match, and recall applies to the current input without switching it. Picture-mode/calibration-bank selectors are not changed either.
4. Choose **Recall settings…**, confirm the physical HDMI signal/bit depth and display context, then **Apply saved state now**. This explicit action sends changes regardless of the On Apply / Immediately preference. It queries current values and uses the normal readback-checked updates. Unavailable settings are skipped and listed, not filled with defaults. If the TV rejects a value and readback confirms the setting and context stayed unchanged, that value is also skipped and the remaining settings continue. For example, a rejected saved Sound mode of Standard is not substituted for ExternalStandard. A rejected required calibration mode skips its dependent rows. Failed final-mode restoration is reported too. Power-off, connection loss, cancellation or an uncertain write still stops recall.
...
**Stop** cancels remaining commands, not changes already sent. If recall is interrupted, the **Recall settings** popup shows the interrupted update and requested calibration modes. Check these on the display, then choose **I checked the TV — close recall review** to acknowledge both reviews in one place. This sends no commands, retries, or undo. Use **Refresh state** before further adjustments. Nothing resumes automatically at startup.

States are private JSON files in `ip-remote/saved-states` under the [data folder](#private-data-command-line-and-updates). Filenames start with your state name and display model, followed by a UTC timestamp and unique identifier—for example, `Evening calibration - S95F - 2026-09-16_143000Z - … .json`. Existing GUID-only files are renamed when the library loads; their contents are not rewritten. Names are sanitized and cannot specify a directory. Duplicate names for the same display/picture-mode/signal context are rejected to avoid silently replacing a calibration.

In **Recall settings**, select a state and click **Show in Finder** (macOS), **Show in File Explorer** (Windows), or **Show in file manager** (Linux). macOS and Windows select the saved file; Linux opens its containing folder using `xdg-open` (a graphical desktop is required). This opens on the computer running the server, not a remote browser's computer. The filename is also shown in the popup; the full folder path is under **Where are my states saved?** Files survive app updates, contain no pairing token, and are not bundled with the app. Back up this folder if wanted; deleting a state is permanent. Older saves that contain an input can also be recalled on a different input.
