...
Choose **Save profile** to persist all desired values, including every indexed grid cell, in the user's local application settings. **Reset & apply all** requires a verified reset confirmation route, asks for destructive confirmation, executes the selected reset choice, returns to normal video, restores declared factory-default predictions, and reapplies every profile value that differs. This resets only the scope of the selected TV menu command—normally **Reset Picture**—and is not the television's full device/ownership factory reset.

Choose **Stay on last adjusted item** to leave the final control visible for inspection, or **Exit to normal video** to run the verified return anchor afterward. Route verification proves that a row can be reached; it does not prove the displayed setting value because the Samsung remote channel does not report values. Visually confirm up to three different sliders to promote their common behavior for the saved TV address. Selection controls use one representative confirmation per interaction type: ordinary selection, submenu selection, and indexed selection. The declared option lists remain part of the file fingerprint, but they do not create one manual check per dropdown. Both results persist across restarts, and controls sharing a verified behavior no longer ask for routine confirmation. Use **Reset sliders** or **Reset selections** if later results disagree. Saving different system-wide menu delays resets both kinds of coverage because those delays affect their key sequences. Switches continue to request routine visual confirmation. If the TV was adjusted outside SamsungController, first restore the declared defaults or update the definition before relying on relative changes.
