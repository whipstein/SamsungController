...
1. **TV profile:** enter the model number and firmware version first. The default name is generated as `<model> · firmware <version>` and the YAML file ID is generated from those values. You may override either generated value. Add signal type, picture mode, and input/source context, then create the profile. The file is saved under the per-user `menu-definitions` directory and loaded automatically.
...
3. **Menu outline:** select **Edit current branch**, paste or edit the hierarchy using two spaces per level, preview it, and apply it. Matching lines retain stable IDs and new IDs are generated. Keep unlisted items enabled unless intentionally synchronizing the whole branch.
...
Red menu items have no recorded command in the active configuration; yellow items have a draft that still needs verification; normal-color items are verified. Topology-only items may remain red. Open **Fine adjustments for individual menu items** only for a small rename, insertion, deletion, parent change, or TV-order correction. Arrow keys navigate this optional editor; the on-row buttons change custom order.
