...
In **Expert settings**, drag a box by its **⠿ Move** handle and drop it at the highlighted edge to rearrange the grid. Related controls move together: Gamma with its adjustment sliders, and Picture Clarity with its motion controls. The **← / →** buttons offer keyboard/touch-friendly ordering. Your layout saves automatically across restarts, including the positions of temporarily hidden settings. **Reset layout** restores the default order. Moving boxes never sends TV commands, changes values, or clears pending edits.
