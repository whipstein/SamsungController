...
**v1 preview branch:** `feature/ip-remote-v1` starts the next major version at `1.0.0-alpha.1`. The **IP Remote · Preview** page pairs over a separate HTTPS connection, reads two state queries, and offers guarded **Contrast, Color, and Sharpness** verification tests. Each control needs its own successful change/readback/visual-check/restore test before **Direct picture controls** unlocks it for that display/context. Explicit Apply keeps the new value after readback; Undo is optional. Target edits preserve the conditions checkbox, and routine Apply/Undo actions have no extra browser popup. Verification evidence persists separately from adjustments, including existing Contrast evidence. All other direct setting writes remain disabled; existing remote/menu controls are unchanged. Follow the [IP Remote setup and staged verification guide](docs/ip-remote-preview.md), including its cancellation/recovery instructions. The stable release remains v0.3.0; this branch will not be merged into `main` until explicitly approved.
