...
The **Menu** page collects adjustable sliders, switches, selections, submenu selections, and indexed selection grids from every verified menu area. It remains locked until the complete menu structure is verified for the selected model, firmware, signal, picture mode, input, and every included configuration. After verification, choose the **Picture**, **Sound**, or another top-level tab, then search within that tab and work with compact cards grouped by their immediate TV-menu section. The separate verified-destination navigator has been removed; use the Menu controls, Remote, macros, or quick-access commands instead.
