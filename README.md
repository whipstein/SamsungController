...
2. Create, duplicate, or edit a macro. Select the verified **Starting TV state** before recording, then choose **Prepare selected start** to move the TV there without adding preparation keys to the draft. New macros default to the currently known state when possible, otherwise to a verified anchor target. The compact editor remote sends buttons to the connected TV and appends each successful send to the draft. Set **Wait after each captured key** to preserve reliable replay timing. Undo or clear captured steps when needed.
