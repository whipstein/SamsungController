...
3. **Menu outline:** the editor is automatically populated from the selected branch in the active YAML. Edit the hierarchy directly using two spaces per level, preview it, and apply it; **Reload branch from YAML** discards unsaved text and restores the saved version. Mark each row as a sub-menu, slider, selection, or switch; value-bearing rows also record their default setting. A selection additionally requires its TV-ordered choices, such as `Picture Mode {selection; default=Filmmaker Mode; options=Standard|Movie|Filmmaker Mode}`. Use `Brightness {slider; default=50}` or `Adaptive Picture {switch; default=off}` for the other value types. A row that stays in place but becomes gray can declare the controlling value, such as `Brightness {slider; default=50; disabledWhen=adaptive-picture=on}`. Matching lines retain stable IDs and new IDs are generated. Keep unlisted items enabled unless intentionally synchronizing the whole branch.
...
Tune **System-wide timing** for default, screen-change, and return waits. Select a known traversal and use its test control before accepting the profile. A draft's **Timing lab** can override the wait after an individual button.

Red menu items have no recorded command in the active configuration; yellow items have a draft that still needs verification; normal-color items are verified. Topology-only items may remain red. Open **Fine adjustments for individual menu items** for a small rename, insertion, deletion, parent change, TV-order correction, or to edit a selection's ordered available choices. A selection's default is chosen from its list. Arrow keys navigate this optional editor; the on-row buttons change custom order.
