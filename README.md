...
For defaults that differ by color format, bit depth, or HDMI input, edit the control in **Build & Verify → Define menu tree → Signal-dependent defaults**. Add conditions and a value, put specific combinations first, then Save/Update. The matching default is used for initial estimates and explicit resets; changing the header signal selectors does not send TV commands or overwrite values you have entered. See the [conditional-defaults tutorial](docs/menu-definition-file-format.md#defaults-that-depend-on-hdmi-input-or-signal) for YAML/JSON examples and physical HDMI-input selectors.
