...
Samsung does not report these setting values to the controller. `disabledWhen` keeps a row present but unavailable; `hiddenWhen` removes a row and its descendants when any condition matches. SamsungController updates these predictions for changes it sends and recalculates affected offsets, but it cannot observe changes made with a physical remote or another application. After an outside change, restore the declared defaults or make the same value change in Menu Controls before relying on navigation. Use a separate configuration for reordered layouts or conditions that are not represented by a controllable setting. A configuration's `conditions` text remains a human-readable checklist, not an automatically evaluated expression.
