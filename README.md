...
2. Expand **Define the overall menu tree** and use **Fast setup**. Select **Edit current branch**, paste or edit the indented outline using two spaces per level, preview it, and apply it to the active TV YAML. Matching lines retain their stable IDs; new IDs are generated. Keep unlisted items enabled unless intentionally synchronizing the whole branch. The single-item editor remains available for descriptions and small corrections.
3. If TV selections make rows appear, disappear, or move, create a named **Menu configuration** for each resulting layout, such as `Game Mode off` and `Game Mode on`. Describe the selections that produce it and choose the active configuration before recording or navigating. It is always shown beside **Current menu** at the top. Switching it resets the expected position to unknown.
4. Read the node colors: red means no command has been recorded in the active configuration; yellow means a command exists there but has not passed verification; the normal color means verified. Topology-only nodes can remain red: record only destinations that should be executable.
5. Under **Describe the recording**, choose an anchor or a source and target transition. The saved route is scoped to the active menu configuration, so a key count verified for one visible list is never reused for another. For transitions, **Also record return to normal video** captures the forward and return sequences as one draft with one validation count.
6. Select **Prepare source** when an existing verified route can place the TV correctly, then **Start live recording**. Use the on-screen controller; each successful button is both sent to the TV and added to the sequence. Undo, clear, or cancel as needed, then save the draft.
7. Tune **System-wide timing** for default, screen-change, and return waits. Select a known traversal and use its test control before accepting the profile. A draft's **Timing lab** can override the wait after an individual button.
8. In **Timing lab and visual validation**, prepare the source, replay the draft, and confirm what you saw on the TV. Each draft remembers its own count. Three accepted visual passes make it verified; the third pass returns to normal video by default.

Samsung does not report these setting values to the controller. If a setting is changed with the physical remote or another application, manually select the matching menu configuration before using a verified traversal. A configuration's `conditions` text is a human-readable checklist, not an automatically evaluated expression.
