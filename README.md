...
- Compact menu-wide controls, fixed 20 Point/Custom Color grids, named current-TV states, and saved target profiles
...
Use **Saved current TV states** when the TV already has settings you want SamsungController to treat as its starting point. With **Wait for Apply** selected, enter the values currently visible on the TV, name the state, and select **Save staged values as current**. Loading it later replaces only the prediction baseline and sends no remote commands. Named states—including indexed grid cells—stay in local application settings. **Save profile** separately stores desired target values. **Reset & apply all** remains an explicitly destructive, confirmed operation for users who choose that workflow; reset commands are never required by Display Verification.

Choose **Stay on last adjusted item** to leave the final control visible for inspection, or **Exit to normal video** to run the verified return anchor afterward. Route verification proves that a row can be reached; it does not prove the displayed setting value because the Samsung remote channel does not report values. Visually confirm up to three different sliders to promote their common behavior for the saved TV address. Selection controls use one representative confirmation per interaction type: ordinary selection, submenu selection, and indexed selection. The declared option lists remain part of the local display-verification fingerprint, but they do not create one manual check per dropdown. Both results persist across restarts, and controls sharing a verified behavior no longer ask for routine confirmation. Use **Reset sliders** or **Reset selections** if later results disagree. Saving different system-wide menu delays resets both kinds of coverage because those delays affect their key sequences. Switches continue to request routine visual confirmation. If the TV was adjusted outside SamsungController, load or create a matching saved current-TV state before relying on relative changes.
