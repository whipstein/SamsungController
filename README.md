...
Use **Enter current settings** when the TV already has settings you want SamsungController to treat as its starting point. Enter the values currently visible on the TV—or load a matching `.samsung-calibration.json` file into the current-settings draft—then name the state and select **Save entered values**. This workspace never calls the TV: its draft is separate from staged adjustments, and loading a state only replaces the prediction baseline. Named states—including indexed grid cells—stay in local application settings. **Save profile** separately stores desired target values from **Adjust TV**. **Reset & apply all** remains an explicitly destructive, confirmed operation for users who choose that workflow; reset commands are never required by Display Verification.

The same portable `.samsung-calibration.json` file can be used in either workspace. Load it under **Adjust TV** to stage and locally save desired targets, review the resulting differences, then select **Apply**. Load it under **Enter current settings** to fill the command-free baseline draft, review it, then select **Save entered values**. Both imports validate the file version, menu-definition ID, every node/selector/value, and duplicate keys before changing their respective draft, and neither import sends TV commands. Because sliders and selections are relative and Samsung reports neither their values nor cursor position, only identify a file as current when the TV actually contains those values. See [Calibration files](docs/calibration-target-files.md) for the portable JSON schema and a complete example.
