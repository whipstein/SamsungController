...
3. **Menu outline:** select **Edit current branch**, paste or edit the hierarchy using two spaces per level, preview it, and apply it. Mark each row as a sub-menu, slider, selection, or switch; value-bearing rows also record their default setting. For example, use `Brightness {slider; default=50}` or `Adaptive Picture {switch; default=off}`. A row that stays in place but becomes gray can declare the controlling value, such as `Brightness {slider; default=50; disabledWhen=adaptive-picture=on}`. Matching lines retain stable IDs and new IDs are generated. Keep unlisted items enabled unless intentionally synchronizing the whole branch.
