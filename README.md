...
The page reevaluates every `disabledWhen` and `hiddenWhen` rule as its controlling value changes. A row marked `disabled: true` and its descendants remain visible as unavailable topology and cannot be executed, but the visible row still counts in Up/Down traversal. A hidden row and its descendants disappear from Menu and calculated routes remove them from sibling offsets. Menu definitions are model-, firmware-, input-, signal-, and picture-mode-sensitive. YAML is the default storage format; JSON is also supported with the same schema and validation. See [Menu definition file format](docs/menu-definition-file-format.md) and [Menu definitions](docs/menu-model.md).
...
Use **Enter current settings** when the TV already has settings you want SamsungController to treat as its starting point. Enter the values currently visible on the TV, name the state, and select **Save entered values**. This workspace never calls the TV: its draft is separate from staged adjustments, and loading a state only replaces the prediction baseline. Named states—including indexed grid cells—stay in local application settings. **Save profile** separately stores desired target values from **Adjust TV**. **Reset & apply all** remains an explicitly destructive, confirmed operation for users who choose that workflow; reset commands are never required by Display Verification.

The **Portable target calibration** strip keeps known-good desired values separate from that baseline. Name and download the current desired values as a `.samsung-calibration.json` file, or load a matching file to stage and locally save its targets. Loading validates the file version, menu-definition ID, every node/selector/value, and duplicate keys before changing the draft. It sends no TV commands and never replaces the current-TV baseline. Review the resulting differences, then select **Apply**. Because sliders and selections are relative and Samsung reports neither their values nor cursor position, first load a current-TV state that actually matches the display; alternatively use the explicitly confirmed **Reset & apply all** workflow to establish declared defaults before applying a complete target. See [Calibration target files](docs/calibration-target-files.md) for the portable JSON schema and a complete example.

During **Apply** or **Reset & apply all**, a red sticky progress strip identifies the current phase and key and provides **Cancel active update**. Cancel reaches the shared command token and prevents the next key after any already in-flight send or delay. It deliberately sends no cleanup, anchor, or return-to-video sequence after cancellation. Values known to have completed remain recorded; the interrupted menu position and partially adjusted indexed rows are treated as uncertain, so inspect the screen and record the TV's current settings before resuming.

Choose **Stay on last adjusted item** to leave the final control visible for inspection, or **Exit to normal video** to run the verified return anchor afterward. The Samsung remote channel does not report setting values, so load or create a matching saved current-TV state after changes made with the physical remote or another application. If behavior later disagrees with the verified profile, reopen the affected check under Display Verification rather than approving individual controls on this page.
